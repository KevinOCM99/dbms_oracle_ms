Rem
Rem $Header: rdbms/admin/dvremov.sql /st_rdbms_10.2/2 2009/12/18 15:22:17 jibyun Exp $
Rem
Rem dvremov.sql
Rem
...
Rem
Rem    NAME
Rem      dvremov.sql - This script is used to remove DV metadata
Rem
Rem    DESCRIPTION
Rem      This script needs to be executed as SYSDBA. It will only remove DV 
Rem      related metadata.
Rem
Rem      Before running this script, please make sure:
Rem      1. DV is disabled. 
Rem      2. recyclebin is turned off
Rem
Rem      
Rem    NOTES
...
Rem    jibyun      12/07/09 - Bug 5654048: Drop DVSYS before dropping the DV
Rem                           roles to avoid ORA-600
Rem    jheng       08/21/09 - script used to deinstall DV
Rem    jheng       08/21/09 - Created
Rem
...
-- DROP PASSWORD FILE RELATED sys OBJECTS created in catmach.sql
...
--Sync Rules after deleting DV rules
...
--drop DVSYS user, includes two DDL triggers, dvsys objects, packages 
drop user DVSYS cascade;
drop user DVF cascade;

--drop dv roles created in catmacg.sql
...
--- Grant privileges revoked in catmach.sql
...
--revert the audit options changed in DV
...
NOAUDIT AUDIT SYSTEM BY LBACSYS WHENEVER SUCCESSFUL;
NOAUDIT AUDIT SYSTEM BY LBACSYS WHENEVER NOT SUCCESSFUL;
...
NOAUDIT AUDIT SYSTEM WHENEVER SUCCESSFUL;
NOAUDIT AUDIT SYSTEM WHENEVER NOT SUCCESSFUL;
