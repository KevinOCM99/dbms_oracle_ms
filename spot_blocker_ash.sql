...
spool  /tmp/spot_blocker_node0&instance_id
declare
  ...
  from (
    ...
    from (
      ...
      from (
        ...
        connect by nocycle session_id = prior blocking_session
        and (instance_number  = prior blocking_inst_id or prior blocking_inst_id is null)) bb) cc) dd
  ...
  for rec in analyze_ash_cur(:instance_id) loop
    ...
    for b_rec in spot_blocker_cur(rec.instance_number,rec.sample_id) loop
      if b_rec.row_num = 1 then
        ...
        rpad('event',35,' ')||rpad('ptext',35,' ')||rpad('p123',35,' ')||rpad('Blocking_SID',15,' ')||rpad('Blocking_S#',15,' ')||rpad('Blocking_Inst_ID',19,' ')||rpad('cycle_flag',15,' ')||rpad('Program',30,' '));
        dbms_output.put_line(rpad('---',5,' ')||rpad('-----',10,' ')||rpad('----------',12,' ')||rpad('-------',12,' ')||rpad('------',20,' ')||
        rpad('-----',35,' ')||rpad('-----',35,' ')||rpad('----',35,' ')||rpad('------------',15,' ')||rpad('-----------',15,' ')||rpad('----------------',19,' ')||rpad('----------',15,' ')||rpad('-------',30,' '));
        ...
        rpad(nvl(b_rec.event,' '),35,' ')||rpad(nvl(b_rec.ptext,' '),35,' ')||rpad(nvl(b_rec.p123,' '),35,' ')||' '||rpad(nvl(to_char(b_rec.blocking_session),' '),14,' ')||rpad(nvl(to_char(b_rec.blocking_session_serial#),' '),15,' ')
        ||rpad(nvl(to_char(b_rec.blocking_inst_id),' '),19,' ')||rpad(b_rec.cycle_flag,15,' ')||rpad(b_rec.program,30,' '));
      else
        dbms_output.put_line(rpad(b_rec.row_num,5,' ')||rpad(to_char(b_rec.rat_sum),10,' ')||rpad(to_char(b_rec.session_id),12,' ')||rpad(to_char(b_rec.session_serial#),12,' ')||rpad(nvl(b_rec.sql_id,' '),20,' ')||
        rpad(nvl(b_rec.event,' '),35,' ')||rpad(nvl(b_rec.ptext,' '),35,' ')||rpad(nvl(b_rec.p123,' '),35,' ')||' '||rpad(nvl(to_char(b_rec.blocking_session),' '),14,' ')||rpad(nvl(to_char(b_rec.blocking_session_serial#),' '),15,' ')
        ||rpad(nvl(to_char(b_rec.blocking_inst_id),' '),19,' ')||rpad(b_rec.cycle_flag,15,' ')||rpad(b_rec.program,30,' '));
