-----------------------------------------------------------------------
-- Query Name  : ts.sql                                              --
-- Purpose     : Show All Tablespaces' Usage                         --
-- Main View   : DBA_Tablespaces; DBA_Data_Files; DBA_Free_Space     --
-- Auxiliary   : DBA_Temp_Files; v$Temp_Extent_Pool                  --
-- Revision    : 20140415                                            --
-----------------------------------------------------------------------
set linesize 120 pagesize 100 heading on feedback 1 verify off
...
all_tbs as (select * from data union all select * from temp)
...
--     , decode(sign(real_max_tbs_size-tbs_size),0,'NO',1,'YES','?')   as extensible
 ...
 order by contents,tablespace_name--usage_percentage desc
/
